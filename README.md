...
Diagnostic and sync errors have different meanings: a missing-variable configuration error means a required local `.env` value is absent; a page/API `401` means the email/token pair or token permissions are not accepted; a network error means the runner could not reach Confluence; `0 matching pages` fails closed when local content exists, preventing accidental deletion. Set `CONFLUENCE_ALLOW_EMPTY_SYNC=true` only when intentionally removing all published content. A successful publication query prints the matched page title and ID before sync continues. Attachments and embedded images are not synchronized.
